...
const sendFallbackEmail = async (to, subject, text) => {
  try {
    ...
    const mailOptions = {
      from: `"BloodConnect SOS" <${process.env.EMAIL_USER || 'sos@bloodconnect.org'}>`,
      to,
      subject,
      text,
    };
...
// @desc    Get all active (pending) SOS requests for donors to browse
// @route   GET /api/sos/all-active
// @access  Private (Donor only)
exports.getAllActiveSOS = async (req, res) => {
  try {
    if (req.user.role !== 'donor') {
      return res.status(403).json({ success: false, message: 'Only donors can access active SOS listings' });
    }

    const activeSOS = await SOSRequest.find({ status: 'pending' })
      .populate('requesterId', 'name email phone')
      .sort({ createdAt: -1 });

    // Format output — mark whether this donor is already enlisted
    const formatted = activeSOS.map((reqItem) => {
      const donorEntry = reqItem.respondedDonors.find(
        (rd) => rd.donorId.toString() === req.user._id.toString()
      );
      return {
        _id: reqItem._id,
        ...
        status: reqItem.status,
        createdAt: reqItem.createdAt,
        isEnlisted: !!donorEntry,
        myResponseStatus: donorEntry ? donorEntry.status : null,
      };
    });
    ...
    res.status(500).json({ success: false, message: 'Error retrieving active SOS requests' });
  }
};

// @desc    Donor voluntarily enlists for an SOS request
// @route   PUT /api/sos/:id/volunteer
// @access  Private (Donor only)
exports.volunteerForSOS = async (req, res) => {
  try {
    if (req.user.role !== 'donor') {
      return res.status(403).json({ success: false, message: 'Only donors can volunteer for SOS requests' });
    }
    ...
    if (sosRequest.status !== 'pending') {
      return res.status(400).json({ success: false, message: 'This SOS request is no longer active' });
    }

    // Check if donor is already enlisted
    const existingIndex = sosRequest.respondedDonors.findIndex(
      (rd) => rd.donorId.toString() === req.user._id.toString()
    );

    if (existingIndex > -1) {
      return res.status(400).json({ success: false, message: 'You are already enlisted in this SOS request' });
    }

    // Add donor as accepted volunteer
    sosRequest.respondedDonors.push({
    ...
    // Also create a notification for the requester
    await Notification.create({
      userId: sosRequest.requesterId,
      sosRequestId: sosRequest._id,
      message: `Volunteer donor ${req.user.name} (${req.user.bloodGroup}) has accepted your SOS request for ${sosRequest.bloodGroupNeeded} blood at ${sosRequest.hospitalName}. Contact: ${req.user.phone}`,
      isRead: false,
    });

    res.status(200).json({
      success: true,
      message: 'Successfully volunteered for the SOS request',
      data: sosRequest,
    });
  } catch (error) {
    console.error('Volunteer for SOS error:', error);
    res.status(500).json({ success: false, message: 'Error volunteering for SOS request' });
  }
};

// @desc    Requester sends direct email to a donor about an SOS request
// @route   POST /api/sos/:id/send-email/:donorId
// @access  Private (Requester only)
exports.sendDirectEmail = async (req, res) => {
  try {
    ...
    // Verify ownership — only the requester who created the SOS or an admin can send
    if (sosRequest.requesterId.toString() !== req.user._id.toString() && req.user.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Not authorized to send emails for this SOS request' });
    }
    ...
    if (!donor.email) {
      return res.status(400).json({ success: false, message: 'Donor does not have an email address on file' });
    }

    // Compose and send the email
    const subject = `Urgent Blood Request: ${sosRequest.bloodGroupNeeded} Needed at ${sosRequest.hospitalName}`;
    const emailBody = `Dear ${donor.name},\n\nYou are receiving this direct request from ${req.user.name} through BloodConnect.\n\nEmergency Details:\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n• Blood Group Required: ${sosRequest.bloodGroupNeeded}\n• Hospital: ${sosRequest.hospitalName}\n• Urgency: ${sosRequest.urgency.toUpperCase()}\n• Contact Phone: ${sosRequest.contactPhone}\n• Requester: ${req.user.name} (${req.user.phone})\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\nPlease log in to BloodConnect to accept or respond to this request. Your help could save a life today.\n\nThank you for being a hero.\n\nBest regards,\nBloodConnect Emergency Team`;

    const emailSent = await sendFallbackEmail(donor.email, subject, emailBody);

    if (emailSent) {
      res.status(200).json({
        success: true,
        message: `Email sent successfully to ${donor.name} (${donor.email})`,
      });
    } else {
      res.status(500).json({
        success: false,
