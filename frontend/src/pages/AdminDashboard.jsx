...
const AdminDashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Side-by-side SVG Doughnut distribution chart & Real-time simulated logs */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Doughnut Chart Card (Left) */}
        <div className="lg:col-span-7 bg-white rounded-3xl p-6 shadow-sm border border-gray-150 space-y-4">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest font-display">System Portal Distribution</h3>
              <p className="text-xxs text-gray-500 font-medium">Visual distribution ratio between vetted user accounts</p>
            </div>
            <span className="bg-red-50 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded border border-red-200">
              Live Audits
            </span>
          </div>

          <div className="flex flex-col sm:flex-row items-center justify-around gap-6 py-4 bg-gray-50/40 rounded-2xl border border-gray-100">
            {/* SVG Doughnut */}
            <div className="relative w-36 h-36 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 42 42">
                <circle cx="21" cy="21" r="15.91549430918954" fill="transparent" stroke="#EBEFF3" strokeWidth="4.2"></circle>
                
                {/* Donors ring (Red) */}
                <circle cx="21" cy="21" r="15.91549430918954" fill="transparent" stroke="#C81E3A" strokeWidth="4.5"
                  strokeDasharray={`${
                    (donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round((donors.length / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                      : 40
                  } ${
                    100 - ((donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round((donors.length / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                      : 40)
                  }`}
                  strokeDashoffset="0"
                ...
                {/* Requesters ring (Green/Teal) */}
                <circle cx="21" cy="21" r="15.91549430918954" fill="transparent" stroke="#0D7F6E" strokeWidth="4.5"
                  strokeDasharray={`${
                    (donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round((requesters.length / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                      : 30
                  } ${
                    100 - ((donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round((requesters.length / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                ...
                {/* Blood Banks ring (Amber) */}
                <circle cx="21" cy="21" r="15.91549430918954" fill="transparent" stroke="#D97706" strokeWidth="4.5"
                  strokeDasharray={`${
                    (donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round((bloodBanks.length / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                      : 30
                  } ${
                    100 - ((donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round((bloodBanks.length / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                  ...
                  strokeDashoffset={`-${
                    (donors.length + requesters.length + bloodBanks.length) > 0 
                      ? Math.round(((donors.length + requesters.length) / (donors.length + requesters.length + bloodBanks.length)) * 100) 
                      : 70
                  }`}
                ></circle>
              </svg>
              ...
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="text-lg font-black text-gray-900 font-mono">
                  {donors.length + requesters.length + bloodBanks.length}
                </span>
                <span className="text-[8px] text-gray-400 font-bold uppercase tracking-widest">Total Vetted</span>
              </div>
            </div>
              ...
              <div className="flex items-center">
                ...
                <div className="flex justify-between w-40">
                  ...
                  <span className="text-gray-400 font-mono font-bold">
                    {Math.round((donors.length / (donors.length + requesters.length + bloodBanks.length || 1)) * 100)}%
                  </span>
                </div>
              </div>
              <div className="flex items-center">
                <span className="w-3 h-3 bg-trust-teal rounded-sm mr-2"></span>
                <div className="flex justify-between w-40">
                  <span>Requesters</span>
                  <span className="text-gray-400 font-mono font-bold">
                    {Math.round((requesters.length / (donors.length + requesters.length + bloodBanks.length || 1)) * 100)}%
                  </span>
                </div>
              </div>
              <div className="flex items-center">
                <span className="w-3 h-3 bg-amber-600 rounded-sm mr-2"></span>
                <div className="flex justify-between w-40">
                  <span>Blood Banks</span>
                  <span className="text-gray-400 font-mono font-bold">
                    {Math.round((bloodBanks.length / (donors.length + requesters.length + bloodBanks.length || 1)) * 100)}%
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Audit Log Panel Card (Right) */}
        <div className="lg:col-span-5 bg-white rounded-3xl p-6 shadow-sm border border-gray-150 space-y-4">
          <div>
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest font-display">Administrative Audit Log</h3>
            <p className="text-xxs text-gray-500 font-medium">Real-time system events logging simulation</p>
          </div>
          
          <div className="space-y-3.5 max-h-[10.5rem] overflow-y-auto pr-1">
            <div className="flex items-start space-x-2.5 p-2 rounded-xl bg-gray-50 border border-gray-100 text-xxs font-semibold">
              <span className="text-trust-teal font-mono">10:42 AM</span>
              <span className="text-gray-600">Donor credential approval checklist verified for <strong className="text-gray-900">Jane Doe</strong></span>
            </div>
            <div className="flex items-start space-x-2.5 p-2 rounded-xl bg-gray-50 border border-gray-100 text-xxs font-semibold">
              <span className="text-trust-teal font-mono">10:15 AM</span>
              <span className="text-gray-600">Hospital coordination SOS alert fulfilled at <strong className="text-gray-900">St. John's Center</strong></span>
            </div>
            <div className="flex items-start space-x-2.5 p-2 rounded-xl bg-gray-50 border border-gray-100 text-xxs font-semibold">
              <span className="text-trust-teal font-mono">09:56 AM</span>
              <span className="text-gray-600">Blood Bank Licence registered: <strong className="text-gray-900">BB-5920-IND</strong> verified</span>
            </div>
            <div className="flex items-start space-x-2.5 p-2 rounded-xl bg-gray-50 border border-gray-100 text-xxs font-semibold">
              <span className="text-trust-teal font-mono">09:30 AM</span>
              <span className="text-gray-600">System metrics cron run completed. All sockets active</span>
            </div>
          </div>
        </div>
      </div>
