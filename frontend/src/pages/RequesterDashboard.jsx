...
const RequesterDashboard = () => {
  ...
  const handleSendEmail = async (sosRequestId, donorId, donorName) => {
    const emailKey = `${sosRequestId}_${donorId}`;
    setSendingEmailId(emailKey);
    try {
      const res = await api.post(`/sos/${sosRequestId}/send-email/${donorId}`);
      if (res.data.success) {
        toast.success(res.data.message || `Email sent successfully to ${donorName}!`);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 bg-clinical-bg min-h-screen">
      ...
      {/* Emergency SOS Response Pipeline Stepper */}
      <div className="bg-white rounded-3xl p-6 shadow-xs border border-soft-border space-y-5">
        <div>
          <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Active SOS Emergency Pipeline</h3>
          <p className="text-xxs text-gray-500 font-medium mt-0.5">Real-time status flow of active broadcasts, donor matching, and supply fulfillment</p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-4 relative">
          {/* Step 1 */}
          <div className="flex flex-col items-center text-center p-4 rounded-2xl bg-sos-red/5 border border-sos-red/10 space-y-2 relative">
            <div className="w-8 h-8 rounded-full bg-sos-red text-white flex items-center justify-center text-xs font-black shadow-sm">1</div>
            <div>
              <span className="text-[10px] font-black text-ink-dark block">SOS Triggered</span>
              <span className="text-[8px] text-gray-500 block font-medium leading-tight">Broadcast sent to coordinates radius</span>
            </div>
          </div>
          
          {/* Step 2 */}
          <div className="flex flex-col items-center text-center p-4 rounded-2xl bg-clinical-bg/50 border border-soft-border space-y-2">
            <div className="w-8 h-8 rounded-full bg-ink-dark/10 text-ink-dark flex items-center justify-center text-xs font-black">2</div>
            <div>
              <span className="text-[10px] font-black text-ink-dark block">Donor Alert Ring</span>
              <span className="text-[8px] text-gray-500 block font-medium leading-tight">Socket notifies local available citizens</span>
            </div>
          </div>
          
          {/* Step 3 */}
          <div className="flex flex-col items-center text-center p-4 rounded-2xl bg-clinical-bg/50 border border-soft-border space-y-2">
            <div className="w-8 h-8 rounded-full bg-ink-dark/10 text-ink-dark flex items-center justify-center text-xs font-black">3</div>
            <div>
              <span className="text-[10px] font-black text-ink-dark block">Response Logged</span>
              <span className="text-[8px] text-gray-500 block font-medium leading-tight">Donors review and accept mission</span>
            </div>
          </div>
          
          {/* Step 4 */}
          <div className="flex flex-col items-center text-center p-4 rounded-2xl bg-clinical-bg/50 border border-soft-border space-y-2">
            <div className="w-8 h-8 rounded-full bg-ink-dark/10 text-ink-dark flex items-center justify-center text-xs font-black">4</div>
            <div>
              <span className="text-[10px] font-black text-ink-dark block">Bank Stock Offer</span>
              <span className="text-[8px] text-gray-550 block font-medium leading-tight">Verified institutions offer held stocks</span>
            </div>
          </div>
          
          {/* Step 5 */}
          <div className="flex flex-col items-center text-center p-4 rounded-2xl bg-clinical-bg/50 border border-soft-border space-y-2">
            <div className="w-8 h-8 rounded-full bg-ink-dark/10 text-ink-dark flex items-center justify-center text-xs font-black">5</div>
            <div>
              <span className="text-[10px] font-black text-ink-dark block">Final Dispatch</span>
              <span className="text-[8px] text-gray-550 block font-medium leading-tight">Acceptance triggers direct contact routing</span>
            </div>
          </div>
        </div>
      </div>
                        ...
                        {req.respondedDonors.length === 0 ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {req.respondedDonors.map((donorEntry) => {
                              ...
                              const emailKey = `${req._id}_${donor._id}`;
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      onClick={() => handleSendEmail(req._id, donor._id, donor.name)}
                                      disabled={sendingEmailId === emailKey}
                                      title={`Send direct email to ${donor.name}`}
                                      className="bg-brand-red/5 hover:bg-brand-red/10 text-brand-red border border-brand-red/15 font-bold p-2 rounded-xl text-xs flex items-center justify-center space-x-1 transition duration-155 disabled:opacity-50"
                                    >
                                      {sendingEmailId === emailKey ? (
                                        <div className="w-3.5 h-3.5 border-2 border-brand-red border-t-transparent rounded-full animate-spin"></div>
                                      ) : (
                                        <>
