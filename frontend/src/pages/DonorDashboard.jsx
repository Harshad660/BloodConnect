...
const DonorDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('alerts'); // 'alerts', 'allActive', 'profile', 'compatibility', 'guide'
  const [isCardFlipped, setIsCardFlipped] = useState(false);
  const [allActiveSOS, setAllActiveSOS] = useState([]);
  ...
  const fetchAllActiveSOS = async () => {
    setAllActiveLoading(true);
    try {
      const res = await api.get('/sos/all-active');
      if (res.data.success) {
        setAllActiveSOS(res.data.data);
      }
    } catch (err) {
      console.error('Error fetching all active SOS:', err);
    } finally {
      setAllActiveLoading(false);
    }
  };

  const handleVolunteer = async (sosId) => {
    setVolunteeringId(sosId);
    try {
      const res = await api.put(`/sos/${sosId}/volunteer`);
      if (res.data.success) {
        toast.success('Successfully volunteered! The requester has been notified.');
        fetchAllActiveSOS();
        fetchIncomingAlerts();
      }
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data?.message || 'Failed to volunteer for this SOS request.');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 bg-clinical-bg min-h-screen">
      ...
      <div className="flex border-b border-soft-border overflow-x-auto whitespace-nowrap scrollbar-none gap-2">
        ...
        <button
          onClick={() => { setActiveTab('allActive'); fetchAllActiveSOS(); }}
          className={`py-3.5 px-6 text-xs uppercase tracking-wider font-extrabold border-b-2 transition duration-155 flex items-center space-x-2 ${
            activeTab === 'allActive'
              ? 'border-brand-red text-brand-red font-black'
              : 'border-transparent text-gray-400 hover:text-ink-dark'
        ...
        >
          ...
          <span>All Active SOS</span>
          <span className={`px-2 py-0.5 rounded-full text-xxs font-extrabold ${
            activeTab === 'allActive' ? 'bg-brand-red/5 text-brand-red border border-brand-red/10' : 'bg-clinical-bg text-gray-500'
          }`}>
            {allActiveSOS.length}
          </span>
        </button>
      ...
      {activeTab === 'alerts' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            {/* Donation Impact Activity Tracker (SVG Chart) */}
            <div className="bg-white rounded-3xl p-6 shadow-xs border border-soft-border space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Donation Impact Tracker</h3>
                  <p className="text-xxs text-gray-500 font-medium mt-0.5">Visual representation of saved lives & hospital sync frequencies</p>
                </div>
                <span className="bg-trust-teal/10 text-trust-teal text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center border border-trust-teal/20">
              ...
              {/* SVG Area Line Chart */}
              <div className="relative h-44 w-full bg-clinical-bg/50 rounded-2xl border border-soft-border p-4 overflow-hidden flex items-end">
                {/* Y-axis labels */}
                <div className="absolute left-3 top-3 bottom-8 flex flex-col justify-between text-[9px] text-gray-400 font-mono font-bold">
                  <span>9 Lives</span>
                  <span>6 Lives</span>
                  <span>3 Lives</span>
                  <span>0</span>
                ...
                {/* SVG Graph */}
                <svg className="w-full h-full pl-8 pb-4" viewBox="0 0 500 120" preserveAspectRatio="none">
                  {/* Grid Lines */}
                  <line x1="0" y1="20" x2="500" y2="20" stroke="#EBEFF3" strokeWidth="1" strokeDasharray="4" />
                  <line x1="0" y1="50" x2="500" y2="50" stroke="#EBEFF3" strokeWidth="1" strokeDasharray="4" />
                  <line x1="0" y1="80" x2="500" y2="80" stroke="#EBEFF3" strokeWidth="1" strokeDasharray="4" />
                  <line x1="0" y1="110" x2="500" y2="110" stroke="#EBEFF3" strokeWidth="1.5" />
                  
                  {/* Gradient definition */}
                  <defs>
                    <linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#C81E3A" stopOpacity="0.2" />
                      <stop offset="100%" stopColor="#C81E3A" stopOpacity="0.0" />
                    </linearGradient>
                  </defs>
                  
                  {/* Filled Area */}
                  <path
                    d="M0,110 L80,100 L160,105 L240,70 L320,85 L400,45 L480,55 L500,30 L500,110 Z"
                    fill="url(#chartGrad)"
                  />
                  
                  {/* Line path */}
                  <path
                    d="M0,110 L80,100 L160,105 L240,70 L320,85 L400,45 L480,55 L500,30"
                    fill="none"
                    stroke="#C81E3A"
                    strokeWidth="3"
                    strokeLinecap="round"
                  ...
                  {/* Pulse and Node dots */}
                  <circle cx="240" cy="70" r="4" fill="#C81E3A" stroke="white" strokeWidth="1.5" />
                  <circle cx="400" cy="45" r="4" fill="#C81E3A" stroke="white" strokeWidth="1.5" />
                  <circle cx="500" cy="30" r="5" fill="#C81E3A" stroke="white" strokeWidth="2" />
                </svg>
                
                {/* X-axis labels */}
                <div className="absolute left-11 right-4 bottom-2 flex justify-between text-[9px] text-gray-400 font-mono font-bold">
                  <span>Jan-Feb</span>
                  <span>Mar-Apr</span>
                  <span>May-Jun</span>
                  <span>Jul-Aug</span>
                  <span>Sep-Oct</span>
                  <span>Nov-Dec (Forecast)</span>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-soft-border shadow-xs">
            <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center space-x-2">
              ...
              <span>All Active Emergency Broadcasts ({allActiveSOS.length})</span>
            </h2>
            <button
              onClick={fetchAllActiveSOS}
              className="text-gray-405 hover:text-brand-red p-2 rounded-xl hover:bg-brand-red/5 border border-soft-border transition duration-155"
              title="Refresh"
            >
              <RefreshCw className="h-4 w-4" />
          ...
          {allActiveLoading ? (
            <div className="flex justify-center items-center py-20 bg-white rounded-3xl border border-soft-border shadow-xs">
              <div className="flex flex-col items-center space-y-2">
                <div className="w-8 h-8 border-2 border-brand-red border-t-transparent rounded-full animate-spin"></div>
                <span className="text-xxs font-bold text-gray-400 font-mono">Loading all active SOS requests...</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-16 border border-soft-border text-center space-y-3 shadow-xs">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-400 max-w-sm mx-auto font-medium">
                  There are currently no pending emergency blood requests in the system.
                </p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {allActiveSOS.map((sos) => (
                <div
                  key={sos._id}
                  className="bg-white rounded-3xl p-6 shadow-xs border border-soft-border hover:border-brand-red/20 transition duration-200 relative overflow-hidden group"
                >
                  {/* Urgent indicator strip */}
                  <div className={`absolute top-0 inset-x-0 h-1 ${
                    sos.urgency === 'critical' ? 'bg-sos-red animate-pulse' : sos.urgency === 'medium' ? 'bg-amber-500' : 'bg-gray-300'
                  }`}></div>

                  <div className="space-y-4 pt-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="wristband-badge font-mono font-bold text-xs py-1 px-3">
                        {sos.bloodGroupNeeded} Required
                      </span>
                      <span
                        className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-lg uppercase tracking-wider ${
                      ...
                      <span className="text-xxs text-gray-400 font-semibold flex items-center">
                        <Clock className="h-3 w-3 mr-1 text-gray-400" />
                        {new Date(sos.createdAt).toLocaleDateString()} • {new Date(sos.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>

                    <div>
                      <h3 className="text-base font-black text-ink-dark font-display flex items-center space-x-1.5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs border-t border-soft-border pt-3">
                      <div className="flex items-center space-x-2 text-gray-655 font-medium">
                        <User className="h-4 w-4 text-gray-400" />
                        <span>Requester: {sos.requester?.name}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-gray-655 font-medium">
                        <Phone className="h-4 w-4 text-gray-400" />
                        <a href={`tel:${sos.contactPhone}`} className="hover:underline font-black text-brand-red font-mono">
                          {sos.contactPhone}
                        </a>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-2.5 pt-2 border-t border-soft-border">
                      {/* Google Maps Link */}
                      {sos.location?.coordinates && (
                        <a
                          href={`https://www.google.com/maps/search/?api=1&query=${sos.location.coordinates[1]},${sos.location.coordinates[0]}`}
                          target="_blank"
                          rel="noreferrer"
                          className="bg-clinical-bg hover:bg-gray-100 text-ink-dark border border-soft-border font-bold px-4 py-2 rounded-xl text-xs flex items-center space-x-1.5 transition duration-155"
                        >
                      ...
                      {sos.isEnlisted ? (
                        <span className={`text-xxs font-extrabold px-4 py-2 rounded-xl flex items-center space-x-1 border uppercase tracking-wider ${
                          sos.myResponseStatus === 'accepted'
                            ? 'bg-trust-teal/10 text-trust-teal border-trust-teal/20'
                            : sos.myResponseStatus === 'declined'
                            ? 'bg-sos-red/10 text-sos-red border-sos-red/20'
                            : 'bg-clinical-bg text-gray-400 border-soft-border animate-pulse'
                        }`}>
                          <Check className="h-3.5 w-3.5" />
                          <span>{sos.myResponseStatus === 'accepted' ? 'Enlisted' : sos.myResponseStatus === 'declined' ? 'Declined' : 'Pending'}</span>
                        </span>
                      ) : (
                        <button
                          onClick={() => handleVolunteer(sos._id)}
                          disabled={volunteeringId === sos._id}
                          className="bg-trust-teal hover:bg-trust-teal/90 text-white font-extrabold px-5 py-2 rounded-xl text-xs flex items-center space-x-1.5 shadow-sm transition duration-155 disabled:opacity-50"
                        >
                          {volunteeringId === sos._id ? (
      ...
      {activeTab === 'profile' && (
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Left Form Column */}
          <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-soft-border space-y-6">
            <h2 className="text-xs font-black text-gray-400 border-b border-soft-border pb-3 uppercase tracking-widest">
          ...
          {/* Right Digital Card Column */}
          <div className="lg:col-span-5 bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-soft-border space-y-6">
            <h2 className="text-xs font-black text-gray-400 border-b border-soft-border pb-3 uppercase tracking-widest text-center">
              Digital Credentials pass
            </h2>
            
            {/* Digital Donor Card Widget */}
            <div className="space-y-4">
              <span className="block text-xxs font-bold text-gray-400 uppercase tracking-wider text-center">Your Digital Donor Identification</span>
              
              <div 
                onClick={() => setIsCardFlipped(!isCardFlipped)}
                className={`relative w-full h-72 rounded-2xl cursor-pointer preserve-3d duration-700 transition-all ${
                  isCardFlipped ? '[transform:rotateY(180deg)]' : ''
                }`}
                style={{ perspective: '1000px' }}
              >
                {/* Front side */}
                <div className={`absolute inset-0 w-full h-full rounded-2xl p-6 bg-gradient-to-br from-ink-dark via-slate-900 to-zinc-950 border border-white/10 flex flex-col justify-between shadow-xl backface-hidden transition-all duration-300 ${
                  isCardFlipped ? 'opacity-0 pointer-events-none' : 'opacity-100'
                }`}>
                  {/* Subtle watermarked grid */}
                  <div className="absolute inset-0 opacity-5 bg-[radial-gradient(#C81E3A_1px,transparent_1.5px)] [background-size:12px_12px] rounded-2xl"></div>
                  
                  <div className="flex justify-between items-start z-10">
                    <div className="flex items-center space-x-2">
                      <svg className="w-5 h-5 text-brand-red animate-pulse" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 21C16 21 18 17 18 13.5C18 10 12 3 12 3C12 3 6 10 6 13.5C6 17 8 21 12 21Z" fill="currentColor"/>
                      </svg>
                      <span className="font-display font-black text-xs text-white tracking-wider">BLOODCONNECT ID</span>
                    </div>
                    {user?.isVerified && (
                      <span className="bg-trust-teal/20 text-trust-teal text-[9px] font-black px-2 py-0.5 rounded-full border border-trust-teal/40 flex items-center backdrop-blur-xs">
                  ...
                  <div className="z-10 py-2">
                    <span className="text-[10px] text-gray-500 block uppercase font-bold tracking-widest leading-none mb-1">Donor Class Group</span>
                    <div className="flex items-baseline space-x-2">
                      <span className="text-4xl font-black text-white font-mono tracking-tighter">{user?.bloodGroup || 'N/A'}</span>
                      <span className="text-[10px] text-brand-red font-bold uppercase">SAVER TYPE</span>
                    </div>
                  </div>
                  
                  <div className="z-10 border-t border-white/5 pt-3">
                    <span className="text-[9px] text-gray-500 block uppercase font-bold tracking-widest leading-none">Vetted Name</span>
                    <span className="text-sm font-black text-white tracking-tight">{user?.name}</span>
                    <span className="text-[8px] text-gray-400 block font-mono mt-0.5">UID: {user?._id?.substring(0, 12)}...</span>
                  </div>
                  
                  {/* Barcode representation */}
                  <div className="flex flex-col items-center justify-center bg-white/5 p-2 rounded-lg border border-white/5">
                    <div className="flex items-stretch justify-center h-6 w-full space-x-[2px] opacity-75">
                    ...
                    <span className="text-[8px] text-gray-400 font-mono tracking-widest mt-1">BC-{user?._id?.substring(18, 24).toUpperCase() || 'SA-01'}</span>
                  </div>
                </div>

                {/* Back side */}
                <div className={`absolute inset-0 w-full h-full rounded-2xl p-6 bg-gradient-to-br from-slate-900 via-slate-950 to-zinc-950 border border-white/10 flex flex-col justify-between shadow-xl backface-hidden transition-all duration-300 ${
                  !isCardFlipped ? 'opacity-0 pointer-events-none [transform:rotateY(180deg)]' : 'opacity-100 [transform:rotateY(180deg)]'
                }`}>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center pb-2 border-b border-white/5">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Clinical Register Details</span>
                      <span className="text-[9px] text-gray-500 font-mono font-bold">Rev: 2026.07</span>
                    </div>
                    
                    <div className="space-y-2 text-xxs font-medium text-gray-300">
                      <div className="flex justify-between">
                        <span className="text-gray-400">Contact Tel:</span>
                        <span className="font-mono text-white">{user?.phone}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Registered City:</span>
                        <span className="text-white">{user?.city || 'Not Registered'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Last Donation:</span>
                        <span className="text-white font-mono">{user?.lastDonationDate ? new Date(user.lastDonationDate).toLocaleDateString() : 'None Logged'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Status:</span>
                        <span className={`font-bold ${user?.isVerified ? 'text-trust-teal' : 'text-amber-500'}`}>
                          {user?.isVerified ? '✓ Fully Verified' : '○ Pending Audit'}
                        </span>
                      </div>
                    </div>
                  </div>
                  
                  <div className="text-[8px] text-gray-500 text-center leading-relaxed border-t border-white/5 pt-3">
                    This digital credentials card maps active hospital perimeters. Tap again to return to face-plate.
                  </div>
                </div>
              </div>
            </div>
