          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-xs border border-soft-border space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Live Inventory Levels</h3>
                  <p className="text-xxs text-gray-500 font-medium mt-0.5">Visual stock indicator. The red dashed line denotes safety minimum threshold ({lowStockThreshold} units)</p>
                </div>
                <div className="flex space-x-4 text-[10px] font-bold">
                  <span className="flex items-center text-trust-teal"><span className="w-2.5 h-2.5 bg-trust-teal rounded-sm mr-1.5"></span>Adequate</span>
                  <span className="flex items-center text-sos-red"><span className="w-2.5 h-2.5 bg-sos-red rounded-sm mr-1.5"></span>Low Stock</span>
                </div>
              </div>
              
              <div className="relative w-full h-48 bg-clinical-bg/50 rounded-2xl border border-soft-border p-5 flex flex-col justify-end">
                {/* Grid Lines & Labels */}
                <div className="absolute left-4 top-4 bottom-10 flex flex-col justify-between text-[8px] text-gray-400 font-mono font-bold">
                  <span>20u</span>
                  <span>15u</span>
                  <span>10u</span>
                  <span>5u</span>
                ...
                {/* Bars Container */}
                <div className="w-full h-32 pl-8 pr-2 flex justify-between items-end relative">
                  {/* Safety threshold line */}
                  <div 
                    className="absolute left-8 right-2 border-t border-dashed border-sos-red/40 z-10 flex items-center"
                    style={{ bottom: `${Math.min(100, (lowStockThreshold / 20) * 100)}%` }}
                  >
                    <span className="bg-sos-red/10 text-[7px] text-sos-red px-1 rounded -mt-2 font-mono font-black">MIN SAFE LEVEL ({lowStockThreshold}u)</span>
                  </div>

                  {BLOOD_GROUPS.map((bg) => {
                    const units = inventory.find(i => i.bloodGroup === bg)?.units || 0;
                    const isLow = units < lowStockThreshold;
                    const percentHeight = Math.min(100, (units / 20) * 100);
                    
                    return (
                      <div key={bg} className="flex-1 flex flex-col items-center group relative h-full justify-end px-1.5 sm:px-3">
                        {/* Tooltip */}
                        <div className="absolute bottom-full mb-1 bg-ink-dark text-white text-[9px] font-bold py-1 px-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-30 shadow-md font-mono whitespace-nowrap">
                          {units} Units
                        </div>
                        ...
                        <div 
                          className={`w-full rounded-t-md transition-all duration-500 ease-out shadow-sm ${
                            isLow 
                              ? 'bg-gradient-to-t from-sos-red/80 to-sos-red group-hover:from-sos-red group-hover:to-sos-red/90' 
                              : 'bg-gradient-to-t from-trust-teal/80 to-trust-teal group-hover:from-trust-teal group-hover:to-trust-teal/90'
                          }`}
                          style={{ height: `${percentHeight}%` }}
                        ></div>
                        
                        {/* Label */}
                        <span className="text-[10px] font-black text-ink-dark font-mono mt-2">{bg}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                {/* Accreditation and Security Seal Card */}
                <div className="bg-white p-6 rounded-3xl border border-soft-border shadow-xs space-y-4 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 -mt-6 -mr-6 w-24 h-24 bg-trust-teal/5 rounded-full blur-xl"></div>
                  <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest">Compliance & Accreditation</h4>
                  
                  <div className="p-4 rounded-2xl border border-soft-border bg-clinical-bg/50 space-y-3">
                    <div className="flex items-center space-x-3">
                      <div className="relative flex items-center justify-center w-10 h-10 rounded-xl bg-trust-teal/10 text-trust-teal border border-trust-teal/20">
                      ...
                      <div>
                        <span className="text-[10px] text-gray-400 font-bold uppercase block leading-none mb-0.5">Vetting Seal</span>
                        <span className="text-xs font-black text-ink-dark">Certified Organization</span>
                      </div>
                    </div>
                    
                    <div className="text-[10px] text-gray-500 leading-relaxed font-medium pt-1">
                      Verified under Clinical Regulation Licence <strong className="text-ink-dark font-mono">{user?.licenceNumber}</strong>. Authorized for blood bank coordinates collection and distribution operations.
                    </div>
                  </div>

                  <div className="space-y-2.5 text-xxs font-bold text-gray-500">
                    <div className="flex justify-between items-center py-1.5 border-b border-soft-border">
                      <span>NHA Verification Status</span>
                      <span className="text-trust-teal font-extrabold flex items-center">
                        <span className="w-1.5 h-1.5 rounded-full bg-trust-teal mr-1 animate-ping"></span>
                        VERIFIED
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-1.5 border-b border-soft-border">
                      <span>Regulatory Audit Date</span>
                      <span className="text-ink-dark font-mono font-black">2026.04.15</span>
                    </div>
                    <div className="flex justify-between items-center py-1.5">
                      <span>Clinical Safety Score</span>
                      <span className="text-brand-red font-mono font-black">99.4% A+</span>
                    </div>
                  </div>
                </div>
              </div>
